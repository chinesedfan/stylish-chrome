...
chrome.runtime.onMessage.addListener(function (request, sender, sendResponse) {
    ...
    switch (request.type) {
        case 'stylish-login':
          ...
          // const key = payload[0].fbase_key;
          const val = payload[0];
            idb.openDB('firebaseLocalStorageDb', 1, {
                    upgrade: function(db) {
                        db.createObjectStore('firebaseLocalStorage');
                    }
                })
                .then(function (db){
                    db.put('firebaseLocalStorage', val);
                })
            ;
            break;
        case 'stylish-logout':
            console.log('[background.js] - got stylish-logout message');
            idb.openDB('firebaseLocalStorageDb', 1, {
                upgrade: function(db) {
                    db.createObjectStore('firebaseLocalStorage');
                }
