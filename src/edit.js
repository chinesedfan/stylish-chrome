...
function initCodeMirror() {
    ...
    document.getElementById("cm-theme").href = theme == "default" ? "" : "external/codemirror/theme/" + theme + ".css";
    ...
    document.addEventListener("DOMContentLoaded", function() {
        ...
        analyticsEvent("Add_style", "shown");
    });
...
function acmeEventListener(event) {
    ...
    switch (option) {
        ...
        case "theme":
            ...
            var url = chrome.runtime.getURL("external/codemirror/theme/" + value + ".css");
...
function goBackToManage(event) {
    analyticsEvent("Add_style", "sm_back_to_manage");
    if (useHistoryBack) {
        event.stopPropagation();
        event.preventDefault();
        history.back();
...
function beautify(event) {
    if (exports.css_beautify) { // thanks to csslint's definition of 'exports'
        ...
        script.src = "./external/beautify/beautify-css.js";
        script.onload = doBeautify;
    }
    analyticsEvent("Add_style", "sm_beautify");
    function doBeautify() {
...
function initHooks() {
    ...
    document.getElementById("editor.lineWrapping").addEventListener("click",
        function(){ analyticsEvent("Add_style", "sm_word_wrap"); }, false);
    document.getElementById("editor.smartIndent").addEventListener("click",
        function(){ analyticsEvent("Add_style", "sm_indentation"); }, false);
    document.getElementById("editor.indentWithTabs").addEventListener("click",
        function(){ analyticsEvent("Add_style", "sm_tabs_indentation"); }, false);
...
function save() {
    ...
    analyticsEvent("Add_style", "sm_save");
}
...
function showMozillaFormat() {
    analyticsEvent("Add_style", "sm_export_ff");
...
function fromMozillaFormat() {
    analyticsEvent("Add_style", "sm_import_ff");
